...
/mob/living/silicon/ai/proc/deploy_to_shell(var/mob/living/silicon/robot/target)
	...
	for(var/borgie in GLOB.available_ai_shells)
		...
		if(R.shell && !R.deployed && (R.stat != DEAD) && (!R.connected_ai || (R.connected_ai == src) ) ) //&& !(using_map.ai_shell_restricted && !(R.z in using_map.ai_shell_allowed_levels)) )	//VOREStation Edit: shell restrictions //heck off with that
			possible += R
