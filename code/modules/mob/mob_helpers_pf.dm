...
/mob/proc/get_input(var/message, var/title, var/default, var/choice_type, var/obj/required_item)
	if(src.incapacitated() || (required_item && !GLOB.tgui_hands_state.can_use_topic(required_item,src)))
		return null
	var/choice
	...
	if(isnull(choice) || src.incapacitated() || (required_item && !GLOB.tgui_hands_state.can_use_topic(required_item,src)))
		return null
	return choice
