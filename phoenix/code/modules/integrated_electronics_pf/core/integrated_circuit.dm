...
//		to_chat(world,"circtuit check to assembly")
	...
	else if(!CanInteract(user, GLOB.tgui_physical_state))
...
/obj/item/integrated_circuit/tgui_host()
...
/obj/item/integrated_circuit/Topic(href, href_list, state = GLOB.tgui_physical_state)
	if(..())
		return 1
